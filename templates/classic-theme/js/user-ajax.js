jQuery(function ($) {
    ...
    //post job calculation are not working now here is working fine
    ...
    $('.days, .cvs, #edays, .enddate').on('change', function() {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response == "success") {
                    ...
                    //var l = 'https://' + $(location).attr('hostname') + '/post-job'; 
                    var l = window.location.protocol + "//" + location.hostname + '/post-job';
                    location.replace(l);
    ...
    $(document).on('change', '.select_check_active', function (e) {
        e.stopPropagation();
        e.preventDefault();
        var action = "pendingitems";
        var $item = $(this).closest('.ajax-item-listing');
        var data = {action: action, id: $item.data('item-id')};
...
jQuery(document).ready(function($) {
   ...
   jQuery.get("https://ipinfo.io/json", function (location) {
        if (location.region == null) {
            jQuery('input[name=city]').val(location.city);
        }
        else {
            getCityidByCityName(location.country, location.region, location.city);
            ...
            jQuery('input[name=latitude]').val(location.latitude);
            jQuery('input[name=longitude]').val(location.longitude);
            jQuery('input[name=city]').val(location.region);
        }

    }, "json");
    //end of Get current user location
});
...
function selectCVs(id,product_id){
    //alert("sadasd");
    var data = {action: "selectCV", id: id, jobid:product_id};
    $.ajax({
        type: "POST",
        url: ajaxurl,
        data: data,
