...
if(checkloggedin()) {
    ...
    $items = get_items($_SESSION['user']['id'],"saved",false,$_GET['page'],$limit);
    $total_item = get_items_count($_SESSION['user']['id'],"saved");
    $pagging = pagenav($total_item,$_GET['page'],$limit,$link['SAVEDJOBS']);

    // Output to template
    $page = new HtmlTemplate ('templates/' . $config['tpl_name'] . '/ad-saved-approval.tpl');
    ...
    $page->SetParameter ('OVERALL_HEADER', create_header($lang['SAVED_JOBS']));
