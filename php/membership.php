...
if(checkloggedin())
{
    if(isset($_GET['subscription-purchased'])){
      
        $page = new HtmlTemplate ('templates/' . $config['tpl_name'] . '/message.tpl');
        $page->SetParameter ('OVERALL_HEADER', create_header($lang['UPGRADE'])); 
        $page->SetParameter ('HEADING', "Congratulation"); 
        $page->SetParameter ('MESSAGE', "Subscription has been purchased successfully."); 
    ...
    { 
        ...
        if(isset($_POST['payment_method_id']))
        {
            ...
            $person->set_expr('subscription_id',$_POST['upgrade']); 
            $person->save();  
            headerRedirect('?subscription-purchased=yes'); 
            die();  
	...
	else
	{
        ...
        foreach ($rows as $info)
        {
            $sub_types[$info['sub_id']]['Selected'] = 0;
            $sub_types[$info['sub_id']]['id'] = $info['sub_id'];
            $sub_types[$info['sub_id']]['feature'] = $info['feature'];
            $sub_types[$info['sub_id']]['description'] = $info['description'];
            $sub_types[$info['sub_id']]['title'] = $info['sub_title'];
            $sub_types[$info['sub_id']]['recommended'] = $info['recommended'];
            $sub_types[$info['sub_id']]['cost'] = $info['sub_amount'];
            $sub_types[$info['sub_id']]['pay_mode'] = $info['pay_mode'];
