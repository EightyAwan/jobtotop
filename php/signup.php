...
if(isset($_GET['confirm']))
{
    ...
    if($check_confirm)
    {
        ...
        email_template("signup_details",$user_info['id'],' ');

     //   message($lang['SUCCESS'],$lang['THANKSIGNUP'], 'login');
        if($user_info['user_type']=='employer'){
            message($lang['SUCCESS'],$lang['THANKSIGNUP'], 'post-job');
        } else{
            message($lang['SUCCESS'],$lang['THANKSIGNUP'], 'login');
        }
...
if(checkloggedin())
{
 //       header("Location: ".$config['site_url']."dashboard");
    if($user_info['user_type']=='employer'){
        header("Location: ".$config['site_url']."post-job");
    } else {
        header("Location: ".$config['site_url']."dashboard");
    }
...
if(isset($_POST["submit"])) {
    ...
    if($errors == 0) {
        ...
        $insert_user->username = $username[0];
 ...
 //       message($lang['WELCOME'],$lang['WELCOMETOSITE'],'dashboard',false);
         message($lang['SUCCESS'],$lang['POST_REGISTRATION'], 'post-job');
        exit;
    }
}
