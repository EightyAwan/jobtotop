...
if (isset($_GET['action'])){
    if ($_GET['action'] == "send_email_to_subscriber") { email_cron_user(); }
    ...
    if ($_GET['action'] == "searchStateFromCountry") {searchStateFromCountry();}
    if ($_GET['action'] == "submitBlogComment") {submitBlogComment();}
    if ($_GET['action'] == "getCityStateID") { getCityStateID();} 
    if ($_GET['action'] == "getCitiesFromState") {getCitiesFromState();}
}

if(isset($_POST['action'])){
    if ($_POST['action'] == "removeImage") { removeImage(); }
    if ($_POST['action'] == "hideItem") { hideItem(); }
    if ($_POST['action'] == "pendingitems") { pendingitems(); }
    ...
    if ($_POST['action'] == "getCustomFieldByCatID") {getCustomFieldByCatID();}
     if ($_POST['action'] == "selectCV") {selectCv();}
    ...
    if ($_POST['action'] == "new_company") {new_company();}
}


function email_cron_user(){
    if($_GET['cypher'] == md5('admin')){
        echo "<pre>";
        print_r($_SERVER);
        global $config,$lang,$line;
        $sql = "SELECT * FROM `".$config['db']['pre']."user` WHERE active = 1 AND email='musman5264@gmail.com'";
        //$sql = "SELECT * FROM `".$config['db']['pre']."user` WHERE active = 1";
...
function new_company(){
    global $config; 
    if (preg_match('/^[a-zA-Z0-9]/',$_POST['camp_name'])){
        $timenow = date('Y-m-d H:i:s');
        $compName = $_POST['camp_name'];
        $check_companyName = ORM::for_table($config['db']['pre'] . 'companies')
                            ->select('name')
                            ->where('name', $compName)
                            ->count(); 
        if ($check_companyName==0){
            $company = ORM::for_table($config['db']['pre'] . 'companies')->create();
            $company->user_id = $_SESSION['user']['id'];
            $company->name = removeEmailAndPhoneFromString($_POST['camp_name']);
            // if(isset($_POST['company_desc']))
            //     $company->description = $_POST['company_desc'];
            // else
...
function email_contact_seller(){
    ...
    if (isset($_POST['sendemail'])) {
        ...
        if(strlen($iteminfo['comm_email'])>1)
            $item_author_email = $iteminfo['comm_email'];
        else
...
FROM `".$config['db']['pre']."cities` AS c
INNER JOIN `".$config['db']['pre']."subadmin1` AS s ON s.code = c.subadmin1_code and s.active = 1
 WHERE (c.name like '%$dataString%' or c.asciiname like '%$dataString%') and c.country_code = '$sortname' and c.active = 1 and s.id=".$state."
 ORDER BY
  CASE
    WHEN c.name = '$dataString' THEN 1
    WHEN c.name LIKE '$dataString%' THEN 2
  ...
  END ";
    ...
    echo $json = '{"items" : '.json_encode($MyCity, JSON_UNESCAPED_SLASHES).',"totalEntries" : '.$total.'}';
    //echo $json = '{"items" : '.json_encode($MyCity, true).',"totalEntries" : '.$total.',"Query":'.$query.'}';
    die();
}

function searchStateFromCountry()
{
       global $config;
    $dataString = isset($_GET['q']) ? $_GET['q'] : "";
    $sortname = check_user_country();
    ...
    $total = ORM::for_table($config['db']['pre'].'subadmin1')
        ->where(array(
            'country_code' => 'sortname',
            'active' => 1
        ))
    ...
    $sql = "SELECT id,code,name,asciiname 
FROM `".$config['db']['pre']."subadmin1` 
 WHERE (name like '%$dataString%' or asciiname like '%$dataString%') and country_code = '$sortname' and active = 1
 ORDER BY
  CASE
    WHEN name = '$dataString' THEN 1
    WHEN name LIKE '$dataString%' THEN 2
    ELSE 3
  END ";
    $query =  $sql . " limit " . $start . "," . $perPage;
    ...
    foreach ($rows as $row) {
        $state_id = $row['id'];
        $state_code = $row['code'];
        $statename = $row['asciiname'];

        $MyState[$i]["id"]   = $state_id;
        $MyState[$i]["code"]   = $state_code;
        //$MyCity[$i]["text"] = $cityname.", ".$statename;
        $MyState[$i]["text"] = $statename;
        $i++;
    }

    echo $json = '{"items" : '.json_encode($MyState, JSON_UNESCAPED_SLASHES).',"totalEntries" : '.$total.'}';
    die();
}
...
function pendingitems()
{
    ...
    if (trim($id) != '') {
        ...
        $pdo = ORM::get_db();
        if($status == "active"){
            $query = "UPDATE `".$config['db']['pre']."product` set status='pending' WHERE `id` = '".$id."' and `user_id` = '".$_SESSION['user']['id']."' ";
            $query_result = $pdo->query($query);
            echo 1;
        }elseif($status == "pending"){
            $query = "UPDATE `".$config['db']['pre']."product` set status='active' WHERE `id` = '".$id."' and `user_id` = '".$_SESSION['user']['id']."' ";
            $query_result = $pdo->query($query);
            echo 2;
        }
        die();
...
function deleteResume()
{
    ...
    if(isset($_POST['id']))
    {
        ...
        // }
        $pdo = ORM::get_db();
       $sql = 'UPDATE '. $config['db']['pre'].'resumes SET `active`=\'0\' WHERE `id` = '.$_POST['id'].' AND `user_id` = '.$_SESSION['user']['id'];
        $pdo->query($sql);
...
function deleteCompany(){
    ...
    if(isset($_POST['id']))
    {
        ...
        // ->delete_many();
        $pdo = ORM::get_db();
        $sql = 'UPDATE '. $config['db']['pre'].'product SET `active`=0 WHERE `company_id` = '.$_POST['id'].' AND `user_id` = '.$_SESSION['user']['id'].";";
        ...
        $sql .= 'UPDATE '. $config['db']['pre'].'product_resubmit SET `active`=0 WHERE `company_id` = '.$_POST['id'].' AND `user_id` = '.$_SESSION['user']['id'].";";
        ...
        //     ))
        //     ->delete_many();
        echo $sql .= 'UPDATE '. $config['db']['pre'].'companies SET `active`=0 WHERE `id` = '.$_POST['id'].' AND `user_id` = '.$_SESSION['user']['id'];
        $pdo->query($sql);
...
function deleteResumitAd()
{
    ...
    if(isset($_POST['id']))
    {
        ...
        $pdo = ORM::get_db();
        $sql = 'UPDATE '.$config['db']['pre'].'product_resubmit  SET active=0 WHERE `product_id` = '.$_POST['id'].' AND `user_id` = '.$_SESSION['user']['id'];
        $pdo->query($sql);
        // ORM::for_table($config['db']['pre'].'product_resubmit')
        //     ->where(array(
        //         'active'=>1,
...
function selectCv(){
    ...
    if(isset($_POST['jobid']) && isset($_POST['id'])){
        $job =   ORM::for_table($config['db']['pre'].'product')
            ->select('remaining_cvs')
            ->where(array(
                'id' => $_POST['jobid'],
                'active'=>1
            ))
            ->find_one();
        if($job['remaining_cvs']>0){
            $sql = "UPDATE ".$config['db']['pre']."user_applied SET selected=1 WHERE id =".$_POST['id'].";";
            $sql .= "UPDATE ".$config['db']['pre']."product SET remaining_cvs=".($job['remaining_cvs']-1)." WHERE id =".$_POST['jobid'].";";

          $pdo = ORM::get_db();
            if($pdo->query($sql)){
                echo "CV selected seccussfuly..";
                die();
            } else {
              //  echo 0;
                die();
...
function getCityStateID(){
    global $config;
    $pdo = ORM::get_db();
    $sql = "SELECT jc.id cityid, js.id stateid  from job_cities jc join job_subadmin1 js on js.code = jc.subadmin1_code 
            WHERE jc.asciiname ='".$_GET['city']."'  ORDER BY jc.asciiname";
     $rs = $pdo->query($sql);
...
function getCitiesFromState(){
    ...
    $pdo = ORM::get_db();
    $sql = "SELECT jc.id cityid, js.id stateid, jc.asciiname  
    from job_cities jc 
    join job_subadmin1 js on js.code = jc.subadmin1_code
    WHERE js.id  = '".$_POST['stateid']."' ORDER by jc.asciiname ";
    $rs = $pdo->query($sql);
    echo '<select id="job-city" class="with-border city-error-b jobcity" name="city" data-size="50" title="{LANG_SELECT} {LANG_CITY}" >';
