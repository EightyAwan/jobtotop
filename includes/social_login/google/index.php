...
$client_id = "694483931057-ars3veg9hodfjqbsl02kfnhlbvt50l4e.apps.googleusercontent.com";
...
if ($gClient->getAccessToken()) {
    ...
    /* ---- Session Variables -----*/
    $userData = array();
    $requestType = (!empty($_GET['request_type'])?$_GET['request_type']:1);
    $userType = (!empty($_GET['userType'])?$_GET['userType']:null);
    
    $userData = checkSocialUser($gpUserData,$picname,$requestType,$userType);
    
     if($userData['status'] == 3)
    {
        $error = $lang['ACCAEXIST'];
    ...
    if($userData['status'] == 4)
    {
        ...
        echo "<script type='text/javascript'>alert('$error');</script>";
        redirect_parent($config['site_url'] ."signup",true);
        exit();
    }
