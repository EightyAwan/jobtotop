...
function get_country_list($selected="",$selected_text='selected',$installed=1){
    ...
    if($installed){
        $result = ORM::for_table($config['db']['pre'].'countries')
            ->select_many('id', 'code', 'name', 'asciiname', 'languages', 'phone')
            ->where('active', 1)
    ...
    foreach ($result as $info)
    {
        ...
        $countries[$count]['phone'] = $info['phone'];
        if($selected!="")
        {
...
function get_maincategory($selected="",$selected_text='selected'){
    ...
    foreach ($result as $info) {
        ...
        if($config['lang_code'] != 'en' && $config['userlangsel'] == '1'){
            ...
            $cat[$info['cat_id']]['name'] = isset($maincat['title']) ? $maincat['title'] : '';
            $cat[$info['cat_id']]['slug'] = isset($maincat['slug']) ? $maincat['slug'] : '';
        }else{
...
function get_categories_dropdown($lang){
    ...
    foreach($result1 as $info1){
        ...
        if($config['lang_code'] != 'en' && $config['userlangsel'] == '1'){
            ...
            $catname = isset($maincat['title']) ? $maincat['title'] : '';
        }
        ...
        foreach($result as $info){
            ...
            if($config['lang_code'] != 'en' && $config['userlangsel'] == '1'){
                $subcat = get_category_translation("sub",$info['sub_cat_id']);
                $subcat_name = isset($subcat['title']) ? $subcat['title'] : '';
            }else{
...
$where ORDER BY $order_by $pagelimit";
    ...
    if ($result) {
        foreach ($result as $info)
        {
            ...
            $item[$info['id']]['description'] = strlimiter(strip_tags($info['description']),150);
            ...
            $item[$info['id']]['location'] = $info['location'];
...
function payment_success_save_detail($access_token){
    ...
    if($payment_type == "subscr"){
        ...
        if(check_valid_author($item_pro_id)) {
            ...
            unset($_SESSION['quickad'][$access_token]);
            $trans_id='?trans_id='.$now;
            message($lang['SUCCESS'],$lang['PAYMENTSUCCESS'],$link['TRANSACTION'].$trans_id);
...
/**
 * Reset the generate PDF
 *
 * @see using TCPDF
 *
 * @since 3.7.0
 */
function invoice_pdf($trans_id)
{
    //create invoice & save it on server
    ...
    class MYPDF extends TCPDF {
        ...
        public function Header() {
            ...
            // Title
            $this->Cell(0, 25, 'Internal Contact Form', 0, false, 'R', 0, '', 0, false, 'M', 'B');
        }
    }
    ...
    $pdf->SetAuthor('MH');
    $pdf->SetTitle('Internal Contact Form');
...
EOD;
    ...
    ob_end_clean();
    $pdf->Output(__DIR__ . '/storage/payments/example.pdf', 'F');
