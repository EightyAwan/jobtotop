...
function create_header($page_title='',$meta_desc = '',$meta_image = '',$meta_article = false)
{
    ...
    $page = new HtmlTemplate ("templates/" . $config['tpl_name'] . "/general_header.tpl");
    ...
    if(isset($_SESSION['user']['id']))
    {
        ...
        $page->SetParameter ('USERSUBSCRIPTION', $get_userdata['subscription_id']); 
...
function main_footer()
{
    ...
    $page = new HtmlTemplate ("templates/" . $config['tpl_name'] . "/main_footer.tpl");
    $page->SetLoop ('HTMLPAGE', get_html_pages());
    $page->SetParameter('SITE_TITLE', $config['site_title']);
    $page->SetParameter('SITE_LOGO', $config['site_logo']);
    $page->SetParameter('PHONE', $config['contact_phone']);
...
function email_template($template,$user_id=null,$password=null,$product_id=null,$item_title=null,$alt_email=null){
    global $config,$lang,$link;
    ...
    if($user_id != null){
        ...
        $user_role =  $userdata['user_type'];
        if($user_role == 'user'){$user_role ='Job Seeker';}
    ...
    if($template == "signup_details"){
       ...
       // if($user_role == 'employer'){
            $page = new HtmlTemplate();
            $page->html = $config['email_to_admin_sub_signup_details'];
            $page->SetParameter ('USER_ID', $user_id);
            $page->SetParameter ('USERNAME', $username);
            $page->SetParameter ('EMAIL', $user_email);
            $page->SetParameter ('UTYPE', $user_role);
            $page->SetParameter ('USER_FULLNAME', $user_fullname);
            $email_subject_admin = $page->CreatePageReturn($lang,$config,$link);
    
            $page = new HtmlTemplate();
            $page->html = $config['email_to_admin_message_signup_details'];
            $page->SetParameter ('USER_ID', $user_id);
            $page->SetParameter ('USERNAME', $username);
            $page->SetParameter ('EMAIL', $user_email);
            $page->SetParameter ('UTYPE', $user_role);
            $page->SetParameter ('USER_FULLNAME', $user_fullname);
            $page->SetParameter ('COUNTRY', $user_country);
            $email_body_admin = $page->CreatePageReturn($lang,$config,$link);
            email('contact@jobtotop.com',$config['site_title'],$email_subject_admin,$email_body_admin);
            // email($config['sub_admin_email'],$config['site_title'],$email_subject_admin,$email_body_admin);
       // }
        return;
    }
    ...
    if($template == "contact"){
        ...
        // email($config['sub_admin_email'],$config['site_title'],$email_subject,$email_body);
    email('contact@jobtotop.com', $config['site_title'], $email_subject, $email_body);
    }
...
// END UPDATE ONLINE STATUS
        ...
        /*** Email to Employer for Saved Jobs Custom Field **/
        $result = ORM::for_table($config['db']['pre'].'product')
            ->select_many('id','product_name')
            ->where(array(
                'status' => 'saved',
                'emailed' => '0'
            ))
            ...
            // {
                ...
                // {
                    ...
                    if(!empty($info3))
                    {
                        ...
                        $insert_active = ORM::for_table($config['db']['pre'].'emailq')->create();
                        $insert_active->email = $info['user_email'];
                        $insert_active->toname = $name;
                        $insert_active->subject = $email_subject;
                        $insert_active->body = $email_body;
                        $insert_active->save();
        ...
        /*** Email to Employer for Saved Jobs **/
...
function set_user_phCode($country_code){
