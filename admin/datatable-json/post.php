<?php
/*
Copyright (c) 2015 Devendra Katariya (BizDesk/Esystematics.com)
*/
...
if( !empty($params['search']['value']) ){
    if(isset($_GET['status'])) {
        $where .=" WHERE ";
        //$where .=" (`p`.`active`=1 AND `u`.`active`=1 AND `ct`.`active`=1 AND `cat`.`active`=1 AND `c`.`active`=1) AND ";
        $where .=" (`p`.`active`=1 AND `u`.`active`=1 AND `cat`.`active`=1 AND `c`.`active`=1) AND ";
...
$sql = "SELECT p.id,p.product_name,p.created_at,p.status,p.screen_shot,p.featured,p.urgent,p.highlight, p.location as cityname, u.username as username, c.name as company, c.logo as company_image
FROM `".$config['db']['pre']."product` as p
INNER JOIN `".$config['db']['pre']."user` as u ON u.id = p.user_id AND `u`.`active`=1
LEFT JOIN `".$config['db']['pre']."companies` as c ON c.id = p.company_id AND `c`.`active` =1 WHERE  p.active=1 ";
//INNER JOIN `".$config['db']['pre']."cities` as ct ON ct.id = p.city AND `ct`.`active` =1 WHERE  p.active=1 ";
//INNER JOIN `".$config['db']['pre']."catagory_main` as cat ON cat.cat_id = p.category AND `cat`.`active` = 1 // cat.cat_name as catname, 
$sqlTot .= $sql;
$sqlRec .= $sql;
...
foreach ($queryRecords as $row) {
    ...
    else if ($ad_status == "saved"){
        $status = '<span class="label label-warning">Saved</span>';
    }
    ...
    //print_r($value);<a href="#" data-url="panel/post_edit.php?id='.$id.'" data-toggle="slidePanel"  title="Edit" class="btn btn-xs btn-default"> <i class="ion-edit"></i> </a>
