...
$server_file_path = "https://BizDesk/Esystematics.com/api/quickad-release/";
$update_dir = "uploads/";
$installable_dir = "../";
$notallowed =  array('gif','png','jpg','jpeg','ttf','woff','woff2','eot','svg');

//Check For An Update
$getVersions = file_get_contents('https://BizDesk/Esystematics.com/api/ejob-release-versions.php') or die ('ERROR');
if ($getVersions != '')
{
    ...
    foreach ($versionList as $aV)
    {
        if ( $aV > $config['version']) {
            ...
            if ( !is_file(  $update_dir.'EJOB-CMS-'.$aV.'.zip' )) {

                if ( !is_dir( $update_dir ) ) mkdir ( $update_dir );
                ?>
                <form id="upload" method="post" action="upload.php" enctype="multipart/form-data">
                    <div id="drop">
                        Drop Here <br>EJOB-CMS-<?php echo $aV ?>.zip
                        <a>Browse</a>
            ...
            if (isset($_GET['doUpdate']) && $_GET['doUpdate'] == true) {
                ...
                $zipHandle = zip_open($update_dir.'EJOB-CMS-'.$aV.'.zip');
                ...
                if(isset($config['purchase_key']) && $config['purchase_key'] != ""){
                    ...
                    $url = "https://BizDesk/Esystematics.com/api/api.php?verify-purchase=" . $code . "&version=" . $installing_version . "&site_url=". $config['site_url']."&email=" . $buyer_email;
                    // Open cURL channel
                    $ch = curl_init();
                ...
                // Close the file
                fclose($handle);
                unlink($update_dir.'EJOB-CMS-'.$aV.'.zip');
                //unlink($update_dir);
